...
addpath('path_to_your_function_directory');  % Add this line to include the path where generateSPDmatrix.m is located
rng(23);
...
tol          = 1e-10;                    % tolerance for objective function
...
p            = ones(M,1);
pone         = randi([1], M, 1);
R            = generateSPDmatrix(M, eigenMean);  % Ensure this line works
oldJ         = 0;
t            = 1;
...
while t < iterOMax
    ...
    % optimize for p
    p = pone; % [ones(N,1);zeros(M-N,1)]   % antenna selection vector reinitialization
